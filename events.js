...
function renderTasks(tasks) {
  ...
  const tasksHtml = tasks.map(taskTemplate).join("tasks");
...
function manageTasks(event) {
  ...
  // event.target will point to the actual icon clicked on. We need to get the parent li to work with however. HINT: 
  // Remember element.closest()? Look it up if you don't

  // because we added 'data-action="delete"' to each icon in a task we can access a dataset property on our target 
  // (e.target.dataset.action)
  // use that in a couple of if statements to decide whether to run removeTask or completeTask
  const action = event.target.dataset.action;
  const parent = action.target.closest("li");
  if (action === "delete") {
    removeTask(parent);
  } 
  if (action === "complete") {
...
//const button = document.querySelector("#submitTask").addEventListener("click", newTask);
